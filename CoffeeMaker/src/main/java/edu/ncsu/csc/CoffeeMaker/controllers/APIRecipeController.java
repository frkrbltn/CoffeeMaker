...
public class APIRecipeController extends APIController {
     ...
     * recipe to edit (as a path variable)
     *
     * @param recipe
     *            The valid Recipe to be edited.
     * @return Success if the recipe could be edited; an error if the recipe
     *         does not exist
     */
    ...
    public ResponseEntity editRecipe ( @RequestBody final Recipe recipe ) {
        ...
        if ( null == service.findByName( recipe.getName() ) ) {
            ...
            /*
             * int size = recipe.getAllIngredients().size(); for ( final
             * Ingredient i : inventory.getAllIngredients() ) {
